...
with patch('wazuh.common.ossec_uid'):
    ...
    get_agents, get_agent_by_name, get_agents_in_group, get_agents_keys
        ...
        with patch('wazuh.core.core_agent.Agent.group_exists', return_value=group_exists):
            with patch('wazuh.agent.get_agents') as mock_get_agents:
                if group_exists:
                    ...
                    assert (expected_agent == next(affected_agent.values()) for expected_agent, affected_agent in
                            zip(expected_agents, agents.affected_items))
                else:
...
def test_agent_get_agents_keys(test_data, agent_list, expected_items):
    with patch('sqlite3.connect', return_value=test_data.global_db):
        ...
        assert (expected_id == agent_id for expected_id, agent_id in zip(expected_items, agent_keys.affected_items))
